import { normalizeStats, type AquariumModel, type RepositoryStats, type Theme } from "../model.js";
...
const fish = (model: AquariumModel, palette: ThemePalette): string => model.fish.map((item, index) => {
  ...
  const restingX = item.reverse ? 760 - index * 72 : 72 + index * 82;
  return `<g class="swimmer${reverse}" style="--swim:${item.speedSeconds}s;--delay:${item.delaySeconds}s;--lane:${y}px;--rest:${restingX}px;--scale:${item.scale}"><title>${escapeXml(item.label)} · ${escapeXml(item.language)}</title><use href="#px-fish-${item.species}" width="64" height="32" style="color:${color};--fin:${fin};--shine:${shine}"/></g>`;
}).join("");
...
const ciBuoy = (model: AquariumModel, palette: ThemePalette): string => {
  ...
  return `<g class="buoy ${model.ciState}" transform="translate(798 83)"><path d="M31 0h10v18H31z" fill="${palette.outline}"/><rect x="18" y="16" width="36" height="13" fill="${color}"/><path d="M13 29h46l-6 23H19z" fill="#f5f7fa"/><path d="M16 35h40l-2 9H18z" fill="${color}"/><g transform="translate(25 2) scale(.72)">${icon}</g><path d="M35 52h4v31h-4z" fill="${palette.outline}"/><text x="36" y="96" text-anchor="middle" class="tiny">${escapeXml(model.ciState.toUpperCase())}</text><title>${escapeXml(model.ciLabel)}</title></g>`;
};
...
const scene = (theme: Theme, model: AquariumModel): string => {
  const palette = PALETTES[theme];
  const titleId = `title-${theme}`;
  const descId = `desc-${theme}`;
  const description = `${model.repository}: ${model.commits30d} commits in 30 days, ${model.fish.length} contributor fish, ${model.stars} stars. ${model.ciLabel}.`;
  return `
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="900" height="320" viewBox="0 0 900 320" preserveAspectRatio="xMidYMid meet" shape-rendering="crispEdges" role="img" aria-labelledby="${titleId} ${descId}" data-theme="${theme}">
  ...
  <defs>
    ...
    ${symbolDefs(palette)}
    <style>
      text{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;fill:${palette.text}}.heading{font-size:18px;font-weight:700;letter-spacing:.5px}.meta{font-size:11px;fill:${palette.muted}}.legend{font-size:10px;fill:${palette.text}}.tiny{font-size:8px;font-weight:700;fill:${palette.text}}
      .surface{animation:surface 5s steps(4,end) infinite}.ray-a{animation:ray 8s steps(8,end) infinite}.ray-b{animation:ray 11s steps(8,end) -4s infinite}.far{animation:drift 16s steps(16,end) infinite}.near{animation:drift 10s steps(12,end) reverse infinite}.plant{transform-box:fill-box;transform-origin:bottom center;animation:sway 3.4s steps(4,end) infinite alternate}.plant.alt{animation-delay:-1.7s}.bubble{animation:rise var(--rise) steps(18,end) var(--wait) infinite}.b1{opacity:.55}.b2{opacity:.8}.b3{opacity:.4}
      .swimmer{transform:translate(-80px,var(--lane)) scale(var(--scale));animation:swim var(--swim) linear var(--delay) infinite}.swimmer.reverse{animation-name:swim-reverse}.swimmer.reverse use{transform:translateX(64px) scaleX(-1)}.tail{transform-box:fill-box;transform-origin:right center;animation:tail .45s steps(2,end) infinite}.pearl{animation:twinkle 2.8s steps(3,end) infinite}.p1{animation-delay:-.7s}.p2{animation-delay:-1.4s}.p3{animation-delay:-2.1s}.jelly{animation:jelly 4.2s steps(5,end) infinite}.crab{animation:crab 13s steps(20,end) infinite}.chest.open .lid{transform-box:fill-box;transform-origin:left bottom;animation:chest 4s steps(3,end) infinite}.buoy.in-progress{animation:signal 1.5s steps(2,end) infinite}.buoy.failure{animation:signal .8s steps(2,end) infinite}
      @keyframes surface{50%{transform:translateX(-12px)}}@keyframes ray{50%{opacity:.35;transform:translateX(25px)}}@keyframes drift{to{transform:translateX(35px)}}@keyframes sway{to{transform:skewX(5deg)}}@keyframes rise{from{transform:translateY(15px);opacity:0}15%{opacity:.8}to{transform:translateY(-210px);opacity:0}}@keyframes swim{from{transform:translate(-80px,var(--lane)) scale(var(--scale))}to{transform:translate(980px,var(--lane)) scale(var(--scale))}}@keyframes swim-reverse{from{transform:translate(980px,var(--lane)) scale(var(--scale))}to{transform:translate(-80px,var(--lane)) scale(var(--scale))}}@keyframes tail{50%{transform:scaleX(.55)}}@keyframes twinkle{50%{opacity:.35}}@keyframes jelly{50%{transform:translateY(8px) scaleY(.92)}}@keyframes crab{50%{transform:translateX(90px)}}@keyframes chest{50%{transform:rotate(-7deg)}}@keyframes signal{50%{opacity:.45}}
  ...
  <g clip-path="url(#tank-${theme})">
    ...
    <rect y="75" width="900" height="245" fill="url(#water-${theme})"/>
    <g class="surface" opacity=".8"><path d="M0 74h50v-4h48v4h48v4h52v-4h52v-4h50v4h52v4h48v-4h50v-4h50v4h50v4h52v-4h52v-4h48v4h50v4h48v-4h50v-4h52v4h50v8H0z" fill="${palette.glass}"/></g>
    <path class="ray-a" d="m85 75 82 0 92 204H198z" fill="url(#ray-${theme})"/><path class="ray-b" d="m390 75 58 0 68 178h-45z" fill="url(#ray-${theme})"/><path class="ray-a" d="m690 75 68 0 55 156h-38z" fill="url(#ray-${theme})"/>
    ...
    ${bubbles(model)}
    <g class="jelly" transform="translate(735 126)"><use href="#jelly" width="44" height="54"/></g>
    ${fish(model, palette)}
    ...
    <g class="plant" transform="translate(52 236)"><path d="M10 67V12h8v55zm8-31 20-22v10L18 47zm-8 8L0 28v-9l10 13z" fill="${palette.plant}"/><path d="M14 57V25h4v32z" fill="${palette.plantLight}"/></g>
    <g class="plant alt" transform="translate(590 225)"><path d="M12 78V15h8v63zm8-37 21-24v12L20 52zM12 51 0 35V23l12 15z" fill="${palette.plant}"/><path d="M16 65V24h4v41z" fill="${palette.plantLight}"/></g>
    ...
    <g class="crab" transform="translate(225 278)"><use href="#crab" width="50" height="30"/></g>
    ${pearls(model)}
...
export const renderAquarium = (theme: Theme, stats: RepositoryStats, options: RenderOptions = {}): string =>
  compact(scene(theme, normalizeStats(stats, options)));
